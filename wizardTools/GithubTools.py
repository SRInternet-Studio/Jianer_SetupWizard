...
def get_folder_list(repo, branch="main") -> list:
    """
    获取指定分支中所有的文件夹列表
    """
    print("getting list")
    try:
        contents = repo.get_contents("", ref=branch) # "" 获取根目录的内容
        folder_list = []
        while contents:
...
def get_file_content(repo, repo_file_path, branch="main"):
    """
    获取远程文件内容
    """
    file_content = repo.get_contents(repo_file_path, ref=branch)
